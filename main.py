...
#adicioonando personagens e objetos, além de adiciona-los em alguns grupos(trazendo as classes):
...
#Função para reiniciar o jogo (pode ser melhorada):
def reiniciar_jogo(): 
...
clique_som = pygame.mixer.Sound('sons/mixkit-click-melodic-tone-1129.wav')
...
#Flag para colocar delay no tiro:
tiro = True 
...
#Loop principal de jogo:
    while gameLoop:
        ...
        relogio.tick(100) #Tick utilizado no jogo.
        for event in pygame.event.get():
            ...
            #Caso haja colisão, aí sim será possível reiniciar o jogo com a tecla R ou fechar com a tecla X:
            if event.type == pygame.KEYDOWN:
                if colidiu == True or alternativa2 == True: #Só acontecerá os eventos abaixo se colidiu for True ou se ele conseguiu obter os pontos para vitória.
                    if event.key == pygame.K_r:  #Evento Responsável para reiniciar o jogo a partir da tecla R
                        reiniciar_jogo()
                        clique_som.play()
                    if event.key == pygame.K_x: #Evento responsável pelo fechamento do jogo a partir da tecla X
                        gameLoop = False
                        tela_inicial_loop = True
                        goku.sound_morte.stop()
                        clique_som.play()
        ...
        if colisoes and colidiu == False or freeza.pontos == 5: #Caso haja colisão ou uma certa quantidade de Freeza's passem, o jogo para de atualizar os sprites.
        ...
        if colidiu == True or pontos >= 50: #Carregamento do 'game over' ou do 'you win':
            if alternativa: #Caso tenha colidido com o Freeza ou deixado 5 passarem.
                tela.fill(preto)
                ...
                fechar = pygame.image.load('imagens/fechar_tecla.png')
                tela.blit(fechar,(largura//2 - 150, altura//2 + 140))
            elif alternativa2: #Caso tenha matado todos os clones de Freeza.
                tela.fill(preto)
        ...
        if colisao_tiro:
            ...
            freeza.velocidade_freeza += 0.1 #aumentando a dificuldade do jogo
            if freeza.velocidade_freeza >= 6.5: #limitando a velocidade
                freeza.velocidade_freeza = 6.5
        ...
        if pontos == 50:
            alternativa2 = True
            #som_vitoria = pygame.mixer.Sound('sons/efeito_vitoria.mp3')
            som_vitoria = pygame.mixer.Sound('sons/LASER_BU.WAV')
