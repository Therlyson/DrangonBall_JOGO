...
class Goku(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if controle[pygame.K_w] or controle[pygame.K_UP]:
            self.velocidade -= self.aceleração
        if controle[pygame.K_s] or controle[pygame.K_DOWN]:
            self.velocidade += self.aceleração
        else:
...
class Arvores(pygame.sprite.Sprite):
    ...
    def update(self):
        if self.rect.topright[0] < 0: #Condição para a arvore após sair pela esquerda, voltar pela direita.
            self.rect.x = largura
        self.rect.x -= velocidade_jogo #Faz a arvore se movimentar para a esquerda
